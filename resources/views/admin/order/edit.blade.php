...
@section('content')
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Order / Edit / #{{ $order->id }}</h1>
        <div class="btn-toolbar mb-2 mb-md-0">
            <a href="{{ route('adm_order_edit', [app()->getLocale(), 'id' => $order->id]) }}" class="btn btn-outline-success"><i class="fa fa-edit"></i> Edit order</a>
        </div>
    </div>
            ...
            <form action="{{ route('adm_order_edit_process', [app()->getLocale(), 'id' => $order->id]) }}" method="POST" enctype="multipart/form-data" >
                ...
                <div class="form-group">
                    ...
                    <select name="d_order_status_id" class="form-control" id="idStatus" >
                        ...
                        <option value="{{ $item->id }}" {{ $item->id == $order->d_order_status_id ? 'selected="selected"' : '' }} >{{ $item->id }} - {{ $item->name }}</option>
                        <?php endforeach; ?>
                    </select>
                ...
                <button type="submit" class="btn btn-primary"><i class="fa fa-edit"></i> Save order</button>
